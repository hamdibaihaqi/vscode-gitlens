...
export class DraftService implements Disposable {
	...
	async createDraft(repository: Repository, baseSha: string, contents: string): Promise<Draft | undefined> {
		...
		// POST v1/drafts
		const draftRsp = await this.connection.fetch(
		...
		const changesetRsp = await this.connection.fetch(
	...
	async getDrafts(): Promise<Draft[]> {
		...
		const rsp = await this.connection.fetch(Uri.joinPath(this.connection.baseGkApiUri, '/v1/drafts').toString(), {
			method: 'GET',
		});
